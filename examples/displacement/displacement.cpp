...
public:
	...
	{
		windowTitle = "Tessellation shader displacement";
	...
	void loadAssets()
	{
		...
		plane.loadFromFile(getAssetPath() + "models/displacement_plane.gltf", vulkanDevice, graphicQueue, glTFLoadingFlags);
	...
	void buildCommandBuffersForMainRendering()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			VkRect2D scissor = vks::initializers::GenRect2D(splitScreen ? width / 2 : width, height, 0, 0);
	...
	void setupDescriptors()
	{
		...
		std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
			...
			vks::initializers::GenDescriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 1),
		};
		...
		std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
			...
			vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &colorHeightMap.descriptorImageInfo),
		};
