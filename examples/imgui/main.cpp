...
public:
	...
	void initResources(VkRenderPass renderPass, VkQueue copyQueue, const std::string& shadersPath)
	{
		...
		if (device->IsExtensionSupported(VK_KHR_DRIVER_PROPERTIES_EXTENSION_NAME))
		{
	...
	void drawFrame(VkCommandBuffer commandBuffer)
	{
		...
		VkViewport viewport = vks::initializers::GenViewport(ImGui::GetIO().DisplaySize.x, ImGui::GetIO().DisplaySize.y, 0.0f, 1.0f);
...
public:
	...
	void setupLayoutsAndDescriptors()
	{
		...
		std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
			vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBufferVS.descriptorBufferInfo),
		};
	...
	void loadAssets()
	{
		...
		models.models.loadFromFile(getAssetPath() + "models/vulkanscenemodels.gltf", vulkanDevice, graphicQueue, glTFLoadingFlags);
		models.background.loadFromFile(getAssetPath() + "models/vulkanscenebackground.gltf", vulkanDevice, graphicQueue, glTFLoadingFlags);
		models.logos.loadFromFile(getAssetPath() + "models/vulkanscenelogos.gltf", vulkanDevice, graphicQueue, glTFLoadingFlags);
	}
