...
public:
	...
	void buildCommandBuffersForMainRendering()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			VkViewport viewport = vks::initializers::GenViewport(
				(float)width,
				(float)height,
				0.0f,
				1.0f);
			...
			VkRect2D scissor = vks::initializers::GenRect2D(
				width,
				height,
				0,
				0);
	...
	void loadAssets()
	{
		...
		models.plane.loadFromFile(getAssetPath() + "models/plane_z.gltf", vulkanDevice, graphicQueue, glTFLoadingFlags);
		models.teapot.loadFromFile(getAssetPath() + "models/teapot.gltf", vulkanDevice, graphicQueue, glTFLoadingFlags);
		models.sphere.loadFromFile(getAssetPath() + "models/sphere.gltf", vulkanDevice, graphicQueue, glTFLoadingFlags);
	}
