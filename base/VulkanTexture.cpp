...
namespace vks
{
	...
	*/
	void Texture2D::loadFromFile(std::string fileName, VkFormat format, vks::VulkanDevice * device, VkQueue copyQueue, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout, bool forceLinear)
	{
		...
		samplerCreateInfo.maxAnisotropy = device->curEnabledDeviceFeatures.samplerAnisotropy ? device->properties.limits.maxSamplerAnisotropy : 1.0f;
		samplerCreateInfo.anisotropyEnable = device->curEnabledDeviceFeatures.samplerAnisotropy;
